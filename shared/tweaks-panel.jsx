...
const __TWEAKS_STYLE = `
  ...
  .twk-panel{position:fixed;right:16px;top:calc(var(--header-h,0px) + 16px);
    ...
    border:none;border-radius:var(--radius-sm);corner-shape:superellipse(1.7);
...
function TweaksPanel({ title = 'Tweaks', noDeckControls = false, children, onOpenChange, renderMobileFooter }) {
  ...
  React.useEffect(() => {
    ...
    if (!body || !open) return;
    let startY = 0, intercepting = false, active = false, points = [];
    ...
    const onTouchMove = (e) => {
      ...
      if (!intercepting) {
        if (body.scrollTop === 0 && dy > 4) intercepting = true;
        else return;
      }
      e.preventDefault();
      if (panelRef.current) {
    ...
    <>
      ...
      <>
        ...
        <div ref={panelRef} className={`twk-panel twk-opening${closing ? ' twk-closing' : ''}`} data-noncommentable="" onAnimationEnd={handleAnimEnd}>
          ...
          <div ref={bodyRef} className="twk-body">
            {children}
