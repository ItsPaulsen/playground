...
const MODS = [
  { name: 'Item Rarity',          param: 'iir',        min: 5,  max: 80,  step: 5,  defaultVal: 25  },
  { name: 'Monster Rarity',       param: 'rarity',     min: 5,  max: 80,  step: 5,  defaultVal: 25  },
...
function CombinedBox({ combined, activeMods }) {
  ...
  async function handleTrade() {
    ...
    try {
      ...
      activeMods.forEach(({ param, value }) => params.set(param, value));
    ...
    <div className={`poe-combined${over ? ' over' : ''}`}>
      <div className="poe-combined-hd">
        <div className="poe-combined-val">
          {combined ? highlightModNames(combined) : <span className="poe-combined-placeholder">Toggle mods below to build your search string</span>}
        </div>
