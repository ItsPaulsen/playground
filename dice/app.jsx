...
const LANG_OPTIONS = [{ value: 'en', label: 'English' }, { value: 'no', label: 'Norsk' }];
...
const STRINGS = {
  en: {
    roll: 'Roll', rollAgain: 'Roll again', noRolls: 'No rolls left',
    newTurn: 'New turn', rollToStart: 'Roll to start', turnOver: 'Turn over', total: 'Total',
    hands: {
      yahtzee: 'Yahtzee!', largeStraight: 'Large straight', smallStraight: 'Small straight',
      fullHouse: 'Full house', fourKind: 'Four of a kind', threeKind: 'Three of a kind',
      twoPair: 'Two pair', pair: 'Pair', none: '—',
    },
  },
  no: {
    roll: 'Trill', rollAgain: 'Trill igjen', noRolls: 'Ingen kast igjen',
    newTurn: 'Ny omgang', rollToStart: 'Trill for å starte', turnOver: 'Ferdig', total: 'Sum',
    hands: {
      yahtzee: 'Yatzy!', largeStraight: 'Stor straight', smallStraight: 'Liten straight',
      fullHouse: 'Hus', fourKind: 'Fire like', threeKind: 'Tre like',
      twoPair: 'To par', pair: 'Par', none: '—',
    },
  },
};
...
function App() {
  ...
  const handName = React.useMemo(
    () => (isYahtzee && hasRolled ? S.hands[scoreHand(shown.map((d) => d.value))] : null),
    [shown, isYahtzee, hasRolled, S],
  );

  const rollLabel = isYahtzee && turnOver ? S.noRolls
    : isYahtzee && hasRolled ? S.rollAgain
    : S.roll;
  ...
  const readoutLabel = isYahtzee ? (turnOver ? S.turnOver : `${rollsUsed} / ${MAX_ROLLS}`)
    : showTotal ? S.total : null;
