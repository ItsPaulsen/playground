function CreditCard({ t }) {
  ...
  React.useEffect(() => {
    ...
    const tick = () => {
      ...
      const EPS  = 0.001;
      const done =
        Math.abs(cur.rx    - tgt.rx)    < EPS &&
        Math.abs(cur.ry    - tgt.ry)    < EPS &&
        Math.abs(cur.scale - tgt.scale) < EPS;

      if (done) { cur.rx = tgt.rx; cur.ry = tgt.ry; cur.scale = tgt.scale; }
      ...
      card.style.boxShadow  = dark
        ? `${shadowX}px ${shadowY}px 70px rgba(255,255,255,.12), 0 4px 16px rgba(255,255,255,.06)`
        : `${shadowX}px ${shadowY}px 70px rgba(0,0,0,.38), 0 4px 16px rgba(0,0,0,.18)`;
    ...
    <div ref={wrapperRef} style={{
      ...
      <div ref={cardRef} style={{
        ...
        <div ref={shineRef} style={{
          ...
          left: (W - shineSize) / 2, top: (H - shineSize) / 2,
        ...
        <div style={{
          ...
          {t.title && (
            <div style={{ color: 'white', fontSize: 20, fontWeight: 500, lineHeight: 1.2, textShadow: '1px 1px 0 rgba(0,0,0,.15)', overflowWrap: 'break-word' }}>
              {t.title}
            </div>
          )}
          {t.subtitle && (
            <div style={{ color: 'rgba(255,255,255,.8)', fontSize: 14, fontWeight: 500, marginTop: 4, textShadow: '1px 1px 0 rgba(0,0,0,.15)', overflowWrap: 'break-word' }}>
              {t.subtitle}
            </div>
          )}
        </div>
